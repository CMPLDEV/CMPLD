...
class OurPartnerController extends Controller{
 ...
 public function __construct(){
  ...
  $this->section_name = "Our Partner";
  $this->file_path = "uploaded_files/our_partner";
 }
 
 public function index(){
  $data = OurPartner::latest()->paginate(50);
  $i = $data->perPage() * ($data->currentPage() - 1);
  return view('admin.our_partner.index', compact('data','i'));
 }
 
 public function add(){
  return view('admin.our_partner.addedit');     
 }
 ...
 public function edit(Request $req){
  $edit = OurPartner::find($req->id);
  return view('admin.our_partner.addedit', compact('edit'));
 }
 
 public function update(Request $req){
  $req->validate(['image'=>'nullable|image|mimes:jpeg,jpg,png,webp|max:150',
                  'title'=> 'nullable|max:200']);
  $data = OurPartner::find($req->id);
  $data->title = $req->title;
  $data->status = $req->status;
