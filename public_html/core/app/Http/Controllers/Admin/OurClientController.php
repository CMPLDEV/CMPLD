...
class OurClientController extends Controller{
 ...
 public function __construct(){
  ...
  $this->section_name = "Our Client";
  $this->file_path = "uploaded_files/our_client";
 }
 
 public function index(){
  $data = OurClient::latest()->paginate(50);
  $i = $data->perPage() * ($data->currentPage() - 1);
  return view('admin.our_client.index', compact('data','i'));
 }
 
 public function add(){
  return view('admin.our_client.addedit');     
 }
 ...
 public function edit(Request $req){
  $edit = OurClient::find($req->id);
  return view('admin.our_client.addedit', compact('edit'));
 }
 
 public function update(Request $req){
  $req->validate(['image'=>'nullable|image|mimes:jpeg,jpg,png,webp|max:150',
                  'title'=> 'nullable|max:200']);
  $data = OurClient::find($req->id);
  $data->title = $req->title;
  $data->status = $req->status;
