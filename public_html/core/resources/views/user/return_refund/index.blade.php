...
<div class="input-group mb-3">
 <select class="form-control" name="order_detail_id" onChange="returnRequest(this.value);">
  ...
  @foreach($returnable_products as $row)
...
@foreach($data as $row)
 <tr>
    <th>#{{$row->order_id}}</th>
    <td><a href="{{url(Str::slug($row->item_name).'.html')}}" target="_blank">{{$row->item_name}}</a></td>
    <td>{!!returnStatus($row->status)!!}</td>
    <td>{{$row->reason}}</td>
    <td>{{$row->description}}</td>
    <td>{!!refundStatus($row->refund_status)!!}</span></td>
...
@else
 ...
 <br>
 <a href="{{route('all-categories')}}" class="order-explore-bt">Explore Now</a>-->
 </h4>
@endif
</div>
</div>
