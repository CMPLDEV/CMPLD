...
@if($data->isNotEmpty())
<hr>
<form action="{{route('return.refund.bottom.action')}}" method="GET" onSubmit="return checkboxValidation()">
 @csrf 
...
@foreach($data as $row)
  <tr>
    ...
    <td>#{{$row->order_id}}</td>
    <td width="200"><a href="{{url(Str::slug($row->item_name).'.html')}}" target="_blank">{{$row->item_name}}</a></td>
    <td>{{$row->reason}}</td>
   <td>{!!returnStatus($row->status)!!}</td>
   <td>
    {!!refundStatus($row->refund_status)!!}</span> 
    @if($row->status == 3 && $row->refund_status != 'REFUNDED')
    </br></br>
    <select class="form-control" name="refund_status" onChange="updateRefundStatus('{{$row->id}}',this.value);">
   ...
   <td>
    {{user($row->user_id)->name}} </br>
    {{user($row->user_id)->email}}
   </td>
   <td>
     <a href="javascript:void(0);" class="text-secondary" data-bs-toggle="tooltip" title="{{$row->description}}"><i class="icon nav-icon" data-feather="eye"></i></a>
     @if(!empty($row->image))
      &nbsp;
      <a href="uploaded_files/return_refund/{{$row->image}}" target="_blank" class="text-secondary"><i class="icon nav-icon" data-feather="paperclip"></i></a>
     @endif
     </br>
