...
<div class="row">
 ...
 <div class="row">
  ...
  <div class="col-lg-7">
   <form class="search-form d-flex align-items-center" method="GET" action="{{route('location.search')}}">
    @csrf  
 ...
 <input type="text" class="form-control @error('search_keyword') is-invalid @enderror" placeholder="Search by name" name="search_keyword" @isset($search_keyword) value="{{$search_keyword}}" @endisset>
  ...
  @if(isset($search_keyword))
     <a href="{{route('admin.location')}}" class="text-danger" data-bs-toggle="tooltip" title="Remove Filter"><i class="icon nav-icon" data-feather="filter"></i></a>
    @endif
...
@if($data->isNotEmpty())
<hr>
<form action="{{route('location.bottom.action')}}" method="GET" onSubmit="return checkboxValidation()">
 @csrf 
...
@foreach($data as $row)
  <tr>
    ...
    <td><a href="{{route('district',$row->id)}}">{{$row->name}} <i class="fa fa-arrow-right"></i> </a></td>
    
    <td>{{date('d-m-Y',strtotime($row->created_at))}}</td>
    
    <td>
      <a href="javascript:void(0);" data-bs-toggle="tooltip" title="Edit" onClick="stateModal('{{$row->id}}');"><i class="icon nav-icon" data-feather="edit"></i></a>
    </td>
  </tr>
