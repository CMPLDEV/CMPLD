...
<div class="row">
 ...
 <a href="{{route('product.offer.add',$product->id)}}" class="float-end btn btn-primary btn-sm" data-bs-toggle="tooltip" title="Add New"> <i class="icon nav-icon" data-feather="plus"></i></a>
 
 <a href="{{route('product')}}" class="btn btn-dark btn-sm" data-bs-toggle="tooltip" title="Go back"> Back</a>
...
@if($data->isNotEmpty())
<hr>
<form action="{{route('product.offer.bottom.action')}}" method="GET" onSubmit="return checkboxValidation()">
 @csrf 
...
@foreach($data as $row)
  <tr>
    ...
    <td>
      ...
      <a href="{{route('product.offer.edit',[$product->id,$row->id])}}" data-bs-toggle="tooltip" title="Edit"> <i class="fa fa-edit"></i></a>
    </td>
  </tr>
@endforeach
</tbody>
