...
@section('title','Showcase Product')
@section('content')
...
<h4 class="mb-0">Showcase Product for "{{$showcase->name}}"</h4>
...
<div class="row">
 ...
 <a href="{{route('showcase')}}" class="btn btn-info btn-sm text-white">Back</a>

 <a href="{{route('showcase.product.add', $showcase->id)}}" class="float-end btn btn-primary btn-sm" data-bs-toggle="tooltip" title="Add New"> <i class="icon nav-icon" data-feather="plus"></i> Add New </a>
...
@if($data->isNotEmpty())
<hr>
<form action="{{route('showcase.product.bottom.action')}}" method="GET" onSubmit="return checkboxValidation()">
 @csrf 
...
@endphp
  <tr>
    ...
    <td>
    <a href="{{route('showcase.product.update.status',[$row->id,$row_st])}}" onClick="return confirm('Are you sure?');">{!!statusBadge($row->status)!!}</a>
    </td>
  </tr>
@endforeach
</tbody>
