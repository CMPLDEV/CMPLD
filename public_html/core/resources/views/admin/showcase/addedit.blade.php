...
@section('title','Add/Edit Showcase')
@section('content')
...
<div class="card">
<div class="card-body">
<h5 class="card-title">@isset($edit) Edit @else Add @endisset Showcase
<a href="{{route('showcase')}}" class="btn btn-info btn-sm float-end text-white">Back</a></h5>

<hr>

<form method="post" enctype="multipart/form-data" @isset($edit) action="{{route('showcase.update')}}" @else action="{{route('showcase.create')}}" @endisset>
 @csrf
...
@if(isset($edit) && !empty($edit->banner))
  <img src="{{asset(showImage($edit->banner,'uploaded_files/showcase'))}}" alt="image" width="100%">
   <br> <a href="{{route('showcase.remove.image',[$edit->id,'banner'])}}" class="text-danger">Remove</a>
 @else 
...
<div class="col-sm-8">
 ...
 <div class="row">
  ...
  <div class="col-sm-12">
 <div class="form-floating mb-3">
 <input type="text" class="form-control @error('url') is-invalid @enderror" id="floatingInput" name="url" placeholder="Enter Link to Redirect" @isset($edit) value="{{$edit->url}}" @else value="{{old('url')}}" @endisset>
