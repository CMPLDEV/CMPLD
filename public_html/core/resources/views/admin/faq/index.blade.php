...
<div class="row">
 ...
 <div class="col-lg-3">
 <a href="{{route('faq.add')}}" class="float-end btn btn-primary btn-sm" data-bs-toggle="tooltip" title="Add New"> <i class="ri-add-circle-line"></i> Add new</a>
</div> 
</div>
</div>
...
@if($data->isNotEmpty())
<hr>
<form action="{{route('faq.bottom.action')}}" method="GET" onSubmit="return checkboxValidation()">
 @csrf 
...
<tbody>
@foreach($data as $row)
@php $row_st = ($row->status==1) ? 0 : 1; @endphp
  <tr>
    ...
    <td><a href="{{route('faq.update.status',[$row->id,$row_st])}}" onClick="return confirm('Are you sure?');">{!!statusBadge($row->status)!!}</a></td>
    <td>
    <td>
      <a href="{{route('faq.edit',$row->id)}}" data-bs-toggle="tooltip" title="Edit"><i class="icon nav-icon" data-feather="edit"></i></a>
    </td>
  </tr>
