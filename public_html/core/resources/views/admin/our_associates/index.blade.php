...
<div class="row">
 ...
 <div class="col-lg-3">
 <a href="{{route('our.associates.add')}}" class="float-end btn btn-primary btn-sm" data-bs-toggle="tooltip" title="Add New"> <i class="ri-add-circle-line"></i> Add new</a>
</div> 
</div>
</div>
...
@if($data->isNotEmpty())
<hr>
<form action="{{route('our.associates.bottom.action')}}" method="GET" onSubmit="return checkboxValidation()">
 @csrf 
...
@foreach($data as $row)
  <tr>
    ...
    <td>
    <img src="{{asset(showImage($row->image,'uploaded_files/our_associates'))}}" alt="{{$row->name}}" width="50" class="rounded"></td>
    
    <td>{{$row->title}}</td>

    <td><a href="javascript:void(0);">{!!statusBadge($row->status)!!}</a></td>
    <td>
      <a href="{{route('our.associates.edit',$row->id)}}" data-bs-toggle="tooltip" title="Edit"><i class="icon nav-icon" data-feather="edit"></i></a>
    </td>
  </tr>
