...
<div class="row">
 ...
 <div class="col-lg-3">
 <a href="{{route('offer.add')}}" class="float-end btn btn-primary btn-sm" data-bs-toggle="tooltip" title="Add New"> <i class="ri-add-circle-line"></i> Add new</a>
</div> 
</div>
</div>
...
@if($data->isNotEmpty())
<hr>
<form action="{{route('offer.bottom.action')}}" method="GET" onSubmit="return checkboxValidation()">
 @csrf 
...
@endphp
  <tr>
    ...
    <td>
    <img src="{{asset(showImage($row->image,'uploaded_files/offer'))}}" alt="{{$row->name}}" width="50" class="rounded">
    {{$row->name}}</td>
    ...
    <td><a href="{{route('offer.update.status',[$row->id,$row_st])}}" onClick="return confirm('Are you sure?');">{!!statusBadge($row->status)!!}</a></td>
    <td>
      <a href="{{route('offer.edit',$row->id)}}" data-bs-toggle="tooltip" title="Edit"><i class="icon nav-icon" data-feather="edit"></i></a>
    </td>
  </tr>
