...
<a href="{{categoryURL($data->slug)}}" class="rm-filter" style="display:none">Remove filter <i class="fas fa-times text-danger"></i></a>
</button>
</div>
@endif
</div>
...
<select name="short" id="sort_by" class="shorting-select" onChange="loadProducts('{{$type}}','{{$type_value}}',this.value);">
...
@foreach($filters as $row)  
 @php $values = explode(',', $row->value); @endphp
 <div class="col-lg-12">
<div class="product__filter-item p-relative">
<div class="accordion" id="categoriesAccordion">
<div class="accordion-item">
<h2 class="accordion-header product__filter-title" id="headingOne">
<button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#filter{{$row->id}}" aria-expanded="true" aria-controls="filter{{$row->id}}">{{$row->key}}</button>
</h2>
<div id="filter{{$row->id}}" class="accordion-collapse collapse" aria-labelledby="headingOne" data-bs-parent="#categoriesAccordion">
...
<input type="checkbox" id="cat-item-{{$value}}{{$row->id}}D" name="{{str_replace(' ','_',strtolower($row->key))}}D[]" class="product_attribute" value="{{$value}}" onClick="filterProduct('{{str_replace(' ','_',strtolower($row->key))}}', '{{$value}}', '{{$type}}', '{{$type_value}}', '{{$sort_by}}');">
<label for="cat-item-{{$value}}{{$row->id}}D">{{$value}}</label>
</div>
@endforeach
</form>
</div>
...
<div class="@if($type == 'category') col-lg-9 @else col-lg-12 @endif">
<!-- /. shop top area -->
...
@foreach($values as $value)
<div class="single-widget-category">
<input type="checkbox" id="cat-item-{{$value}}{{$row->id}}" name="{{str_replace(' ','_',strtolower($row->key))}}[]" class="product_attribute" value="{{$value}}" onClick="filterProduct('{{str_replace(' ','_',strtolower($row->key))}}', '{{$value}}', '{{$type}}', '{{$type_value}}', '{{$sort_by}}');">
<label for="cat-item-{{$value}}{{$row->id}}">{{$value}}</label>
</div>
@endforeach
</form>
</div>
...
let type_value = decodeHtmlEntities("{{$type_value}}");
...
var url = "{{url('/')}}/"+"load-products?type="+type+"&type_value="+type_value+"&sort_by="+sort_by+"&category_id="+category_id+"&filter_data="+filter_data+"&page="+page;
$.ajax({
url: url,
beforeSend:function(){
$('#load-more-btn').fadeOut('fast');  
